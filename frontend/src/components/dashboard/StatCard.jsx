...
const StatCard = ({ title, value, change, isPositive, icon: Icon, color, onClick }) => {
    ...
    return (
        <div
            onClick={onClick}
            className="bg-[var(--bg-card)] rounded-2xl p-6 relative overflow-hidden group hover:-translate-y-1 transition-transform duration-300 cursor-pointer shadow-lg border border-[var(--border-color)]"
        >
            ...
            <div className="relative z-10">
                <div className="flex justify-between items-start mb-4">
                    <div className={`p-3 rounded-2xl ${styles.bg} ${styles.text} bg-opacity-20`}>
                        <Icon size={28} />
                    </div>
